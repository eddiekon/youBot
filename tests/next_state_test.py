...
from test_functions import next_state
...
u = [10,10,10,10]
configs = np.array([config])
for i in range(N):
    next_config = next_state.next_state(config[0:12],u,dt,u_max)
    next_config.append[0]
    configs = np.concatenate(configs,next_config,axis=0)
    config = next_config

np.savetxt(
    "next_state_results.csv",
    configs,                   
    delimiter=",",
    fmt="%.6f",               
